# 用户数据注入<a name="ZH-CN_TOPIC_0032380449"></a>
...
3.  系统创建弹性云服务器，弹性云服务器启动后自动执行Cloud-init或Cloudbase-init组件，读取用户数据脚本。
